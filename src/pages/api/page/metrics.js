    ...
    async (req, res) => {
      if (req.method === 'GET') {
        ...
        if (!resOrg.success) {
          ...
          if (!isEmpty(resOrg.settings?.integrations)) {
            const startDate = req.query.startDate;
            const endDate = req.query.endDate;

            return PageService.getMetrics(
              req.user.uid,
              startDate,
              endDate,
            ...
            res
              ...
              .json({
                success: false,
                message:
                  `You must <a href="/settings/">link your Google Webmaster account</a> to display performance data.`,
              });
          }
        }
      } else {
