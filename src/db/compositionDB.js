...
class ComponentDB {
  ...
  static async getAll(orgId, offset, limit) {
    ...
    const { rows } = await db.query(
      'SELECT component_id AS "componentId", organization_id AS "organizationId", name, settings, date_added AS "dateAdded", date_modified AS "dateModified" FROM component WHERE (organization_id IS NULL OR organization_id=$1) AND deleted=false ORDER BY date_modified DESC OFFSET $2 LIMIT $3',
      values,
    );
    return rows;
  }
